; QRSC PC Final Installer - Complete Version
; Full-featured installer with Japanese UI and all functions
...
AllowNoIcons=yes
OutputDir=Output
OutputBaseFilename=QRSC_PC_Setup_Final
Compression=lzma
...
Name: "autostart"; Description: "Windows起動時に自動起動 (推奨)"; GroupDescription: "システム設定:"
...
; 自動起動設定（選択時のみ）
Filename: "{app}\install_autostart.bat"; Parameters: ""; WorkingDir: "{app}"; Flags: runhidden waituntilterminated; Description: "Windows起動時の自動起動を設定"; Tasks: autostart
...
begin
  ...
  begin
    ...
    MsgBox('QRSC PCのインストールが完了しました！' + #13#10 + #13#10 +
           ...
           '   ※管理者として実行してください' + #13#10 + #13#10 +
           '2️⃣ アプリケーション起動' + #13#10 +
           '   QRSC PCを起動してサーバーを開始' + #13#10 + #13#10 +
           '3️⃣ スマートフォンから接続' + #13#10 +
           '   QRコードまたは手動でIPアドレスを入力' + #13#10 + #13#10 +
           '📖 詳細な使用方法は「使用方法」ショートカットをご確認ください。' + #13#10 +
           '問題が発生した場合は、GitHub Issuesでサポートを受けられます。', 
           mbInformation, MB_OK);
  end;
end;
