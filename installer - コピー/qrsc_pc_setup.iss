...
begin
  ...
  if MsgBox('QRSC PCをアンインストールしますか？' + #13#10 + #13#10 +
            ...
            '• ファイアウォール設定' + #13#10 +
            '• 設定ファイル' + #13#10 + #13#10 +
            '⚠️ QRSC PCが実行中の場合は、先に終了してからアンインストールを実行してください。', 
  ...
  // QRSC PCプロセスを安全に終了（フリーズ防止のため非同期実行）
  Log('QRSC PCプロセスの終了を試行中...');
  try
    if Exec('taskkill', '/IM qrsc_pc.exe', '', SW_HIDE, ewNoWait, ResultCode) then
    begin
      Log('QRSC PCプロセス終了コマンドを実行しました。');
      // 短い待機時間でプロセス終了を待つ
      Sleep(500);
    end
    else
    begin
      Log('taskkillコマンドの実行をスキップしました。');
    end;
  except
    Log('プロセス終了処理でエラーが発生しましたが、アンインストールを続行します。');
  end;
end;
