...
/// HTTPS/WebSocketサーバーを管理するクラス
class EYMHttpsServer {
  ...
  Future<bool> start({int? port}) async {
    ...
    try {
      ...
      _logger.e('HTTPSサーバーの開始に失敗しました', error: e, stackTrace: stackTrace);
      return false;
    }
  }

  /// SSL証明書を準備
  Future<void> _prepareCertificates() async {
    try {
      _logger.i('SSL証明書を準備しています...');
      ...
      _securityContext = SecurityContext()
        ..useCertificateChainBytes(certData.certificatePem)
        ..usePrivateKeyBytes(certData.privateKeyPem);

      _logger.i('SSL証明書の準備が完了しました');
    } catch (e, stackTrace) {
      _logger.e('SSL証明書の準備に失敗しました', error: e, stackTrace: stackTrace);
      throw Exception('SSL証明書の準備に失敗しました: $e');
    }
  }
  ...
  Router _createRouter() {
    ...
    // 従来のAPIとの互換性のため、ルートパスでもショートカット情報を返す
    router.get('/', (Request request) {
      final shortcuts = <String, Map<String, dynamic>>{};
      
      for (final shortcut in _currentConfig.shortcuts) {
        shortcuts[shortcut.buttonId.toString()] = {
          'name': shortcut.name,
          'description': shortcut.name,
          'icon': _getIconName(shortcut),
        };
      }
      
      return Response.ok(
        jsonEncode({'shortcuts': shortcuts}),
        headers: {'Content-Type': 'application/json'},
      );
    });
      ...
      try {
        ...
        return Response.ok(
          ...
          headers: {
            ...
            'ETag': '"${bytes.length}-${iconPath.hashCode}"',
          },
        );
      } catch (e) {
  ...
  Map<String, dynamic> getServerInfo() {
    ...
    return {
      ...
      'port': _port,
      'url': _isRunning ? 'https://$localIP:$_port' : null,
      'localUrl': _isRunning ? 'https://localhost:$_port' : null,
      'wifiUrl': _isRunning ? 'https://$localIP:$_port' : null,
      'adbCommand': 'adb reverse tcp:12345 tcp:$_port',
      'connectionType': 'Wi-Fi & USB (HTTPS)',
  ...
  /// アイコン名を取得（互換性のため）
  String _getIconName(Shortcut shortcut) {
    if (shortcut.iconPath != null && shortcut.iconPath!.isNotEmpty) {
      return shortcut.iconPath!.split(Platform.pathSeparator).last.split('.').first;
    }
    return 'default';
  }
